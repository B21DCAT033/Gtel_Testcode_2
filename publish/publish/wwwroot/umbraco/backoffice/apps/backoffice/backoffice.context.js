...
export class UmbBackofficeContext extends UmbContextBase {
    ...
    constructor(host) {
        ...
        this.consumeContext(UMB_APP_CONTEXT, (appContext) => {
            if (appContext &&
                typeof appContext.showSettingMenu === 'boolean' &&
                typeof appContext.showPackageMenu === 'boolean' &&
                typeof appContext.showMemberMenu === 'boolean') {
                this._appContext = appContext;
            }
            else {
                console.error('AppContext không hợp lệ hoặc thiếu dữ liệu:', appContext);
            }
        });
        this.#init();
    }
    async #init() {
        ...
        this.observe(userContext.allowedSections, (allowedSections) => {
            ...
            const filters = {
                'Umb.Section.Settings': this._appContext?.showSettingMenu ?? false,
                'Umb.Section.Packages': this._appContext?.showPackageMenu ?? false,
                'Umb.Section.Members': this._appContext?.showMemberMenu ?? false
            };
            new UmbExtensionsManifestInitializer(this, umbExtensionsRegistry, 'section', (manifest) => allowedSections.filter((section) => filters[section] !== false).includes(manifest.alias), async (sections) => {
